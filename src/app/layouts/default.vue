...
<template>
  <div class="relative w-screen h-screen overflow-hidden">
    ...
    <!-- 主要内容区域 -->
    <main ref="scrollContainer" class="  w-full h-full scroll-container scroll-smooth">
      <div class="min-h-[200vh] p-8">
        <RouterView />
        <!-- 添加一些内容来测试滚动 -->
        <div class="mt-8 space-y-4">
          <div v-for="i in 50" :key="i"
            class="p-4 bg-gradient-to-r from-pink-400 to-red-500 text-white rounded-lg text-center font-medium shadow-md hover:shadow-lg transition-shadow">
            内容区域 {{ i }}
          </div>
        </div>
      </div>
