...
import { RS, RW } from "../plugins/ResponseSchema";
import { eq } from "drizzle-orm";
import { NotFoundUserError } from "../decorators/base";
...
try {
 ...
 .post('/login',async ({body:{username}})=>{

  const  user =await  db.select().from(userSchema).where(eq(userSchema.username,username)).limit(1)
